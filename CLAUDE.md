...
**Bulk downloads.** Two kinds, and the difference is load-bearing: the *text corpora* are chunked for the reference lane's search tools (and embedded once Phase 1b adds vectors); the *structured* sources are *not* — they land as a lossless columnar mirror that a later typed layer reads. Don't chunk a structured source.

*Text corpora (reference-lane ingestion):*
...
- **Phase 1 — the agent itself, over the reference corpus.** The one PydanticAI `Agent` gets built here and **every later phase registers more tools on it rather than replacing it** — output schema, provenance, grounding rule, and step limits are written once, in this phase. Not a RAG pipeline: nothing chains retrieve → stuff-context → generate; the agent decides which tool to call and when it has enough. Split into **1a** (a *small full-text toolset* the agent composes itself — `list_documents` / `grep_corpus` / `search_corpus` / `get_chunk`, **no database of any kind**: no vector store, no DuckDB/SQLite FTS; stdlib BM25 over an in-memory inverted index, since BM25 is a ranking formula rather than a storage engine) and **1b** (a `vector_search` tool over LanceDB registered **alongside** the 1a tools, not replacing them; the eval compares lexical-only / vector-only / both). Extend eval to answer correctness + groundedness/faithfulness. *Frontend (F1): the Phase 0 stub is swapped for the real agent, streaming wired up, eval dashboard over real runs. 1b adds only an eval run-comparison view — the chat UI is untouched by design.*
- **Phase 2 — Add web search.** A second *lane* on the same agent: an agent-oriented search API (Tavily or Exa, chosen by measurement — not a scraped SERP), so the agent must now choose corpus vs. open web. Add a routing-correctness eval slice, separate from answer correctness. Tag each answer with its source type. *Frontend (F2): the `web` source badge goes live; routing accuracy joins the eval dashboard.*
...
- **Grounding guardrail**: the agent must answer only from what its tools returned and explicitly abstain ("not in my reference material") rather than hallucinate when a question falls outside the corpus.
