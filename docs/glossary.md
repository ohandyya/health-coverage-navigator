...
| **openFDA** | — | FDA's public API at `https://api.fda.gov/`. Usable with no key at 1,000 requests/day per **IP** (a free key would raise that to 120,000, and this repo **deliberately does not request one** — the lookups are per-drug and never approach the limit; [`structured-api-tools.md`](structured-api-tools.md) §4). Supplies drug labels, recalls, and shortages in the `structured_api` lane. It also publishes bulk JSON, which this repo **deliberately does not use** — the questions asked of it are per-drug lookups, so the API is the only route (see [`plan.md`](plan.md)). Always written lowercase-o, camel — `openFDA`, not `OpenFDA`. |
...
| **FOIA** | Freedom of Information Act | Why NPPES provider data *would* be safe to vendor despite naming real practitioners — it is FOIA-disclosable. Moot in practice: NPPES is queried live and never vendored, so no practitioner name reaches this repo. The distinction matters for `scripts/scan_sensitive.py` and its `pii:npi` marker. **The invariant is that no *real* provider identity is vendored** — for a long time that was expressible as "the count stays at zero", and Phase 3 outgrows the proxy: synthetic provider fixtures are Luhn-valid by necessity and so are counted, which is why the precise form is **zero unallowlisted hits, every allowlisted hit synthetic by construction** ([`structured-api-tools.md`](structured-api-tools.md) §8a). |
...
| **Medigap Open Enrollment Period** | — | A one-time, 6-month guaranteed-issue window starting the month someone is both 65+ and has Part B — during it, an insurer must sell any Medigap policy it offers regardless of health history. Distinct from the Marketplace's annual **open enrollment** and from a Medigap policy's own effective date; the gold eval set's `evals/gold/questions.yaml#pub-01` targets this window specifically because, unlike a dollar figure, it doesn't move year to year. |
...
| **Service Area PUF** | — | Maps each plan's `ServiceAreaId` to the counties/ZIP codes it's actually sold in. Not named in `docs/plan.md`'s original two-PUF description, but included in `exchange_puf` anyway: without it, `ServiceAreaId` alone can't answer "which plans are available in ZIP 27360" — the plan's own canonical structured-lookup example. (The vendored data cannot answer it either: `ZipCodes` is populated in 23 of 8,820 rows, so this is county coverage, not ZIP coverage. Phase 3's `find_plans` resolves the ZIP live.) |
...
| **NPI** | National Provider Identifier | The 10-digit ID NPPES assigns to a provider. The lookup key for *"what is this NPI's specialty"* — a `structured_api` question, never a `reference` one. Self-validating: the tenth digit is a Luhn check over the prefix `80840` plus the first nine, which is how `scripts/scan_sensitive.py` tells a real NPI from any other ten-digit run — paired with a second filter requiring the literal word "NPI" nearby, since Luhn alone passes ~10% of arbitrary digit runs. Consequence for test data: a **synthetic** NPI must be Luhn-valid too, or the code's own validation rejects its own fixtures — so synthetic NPIs are generated, allowlisted, and **never written literally into a tracked file**, on the same rule this glossary states for **SSN**. |
...
| **Marketplace API** | — | CMS's API at `marketplace.api.healthcare.gov/api/v1/` for plan search, drug-coverage checks, and cost estimates. Drives Window Shop and Plan Compare on HealthCare.gov. **Requires an API key** from the CMS developer portal; rate-limited, and the key **expires every 60 days** with a replacement emailed automatically ([`structured-api-tools.md`](structured-api-tools.md) §3). |
| **FFM** | Federally Facilitated Marketplace | The HealthCare.gov exchange, and **the only market the Marketplace API serves**. States running their own exchange (a *state-based marketplace*) are refused with `400 "state is not a valid marketplace state"` — CA, GA and NY verified 2026-08-22 ([`structured-api-tools.md`](structured-api-tools.md) §10d). A 400 there is an answer, not an outage: the reader needs their state's own marketplace. |
| **Georgia Access** | — | Georgia's state-based marketplace, which replaced HealthCare.gov for that state. Named here because it is the concrete case behind the **SBM** entry and because the reference corpus predates the move: asked which exchange serves a Georgia ZIP, the agent answered *"Georgia uses HealthCare.gov"* from a vendored document that had stopped being true. The live API is what corrects it ([`structured-api-tools.md`](structured-api-tools.md) §10d). |
| **state-based marketplace (SBM)** | — | An exchange a state runs itself instead of using HealthCare.gov. Consequence for this repo: no live plan or price data is reachable for those states, so a plan-search question about one is answered by naming the situation rather than by abstaining. |
...
| **market year** | — | The plan year a Marketplace query is about. Discoverable rather than assumed: `GET /market-years` returns the supported years and which is `current`, so neither half of the structured lane hardcodes one. Load-bearing for fixtures — a stale year returns `DataNotProvided` where the current year returns real coverage ([`structured-api-tools.md`](structured-api-tools.md) §7c). |
| **FIPS county code** | Federal Information Processing Standards | The 5-digit federal county identifier (e.g. `37057` = Davidson County, NC). Required alongside state and ZIP in the Marketplace API's `Place` object, because a ZIP can straddle counties and premiums are set per county. Resolvable from a ZIP via `GET /counties/by/zip/{zipcode}`. |
| **rate area** | — | The geographic unit an issuer prices against — a state's counties grouped into zones by regulators. Returned by Marketplace plan search as `rate_area`, and why two households in the same state can see different premiums for the same plan. |
| **APTC** | Advance Premium Tax Credit | The ACA premium subsidy, paid to the insurer in advance and reconciled at tax time — distinct from **CSR**, which lowers cost sharing rather than premium. Returned by `POST /households/eligibility/estimates`; also a `Person` flag (`aptc_eligible`) on Marketplace plan search, and the difference between a plan's `premium` and its `premium_w_credit`. |
...
| **boxed warning** | — | The FDA's **most serious** warning class, printed inside a black border at the top of a label — reserved for risks that can lead to death or serious injury. Its own SPL field (`boxed_warning`), separate from `warnings_and_cautions`, and present on only some prescription labels and no over-the-counter ones. Why this repo's `warnings` section gathers *every* candidate field rather than the first one found: dropping a boxed warning because a general warnings section also existed would omit the gravest thing the label says. Still commonly called a *black box warning*, which the FDA does not use. |
...
| **taxonomy code** | — | The NUCC-maintained code identifying a provider's specialty, and how NPPES answers *"what is this NPI's specialty"* — `taxonomies[]` on a provider record, each with `desc`, `state`, `license`, and a `primary` flag. **A provider may hold several and exactly one is primary**, so reading `taxonomies[0]` can report a secondary specialty as the specialty ([`structured-api-tools.md`](structured-api-tools.md) §10a). |
| **enforcement report** | — | The FDA's record of a recall — classification, reason, distribution, status. openFDA's `/drug/enforcement.json`, and the source for *"has drug X been recalled"*. A search matching nothing returns **HTTP 404**, which for this question is the answer (no recalls) rather than a failure. |
...
| **benefit period** | — | A Part A accounting window, not a calendar period: it starts on hospital or SNF admission and ends after 60 consecutive days without inpatient/SNF care, and cost sharing (like the SNF days-21-100 coinsurance in `evals/gold/questions.yaml#pub-02`) resets with each new one. Also used for **hospice benefit periods** — 90-day, then 90-day, then unlimited 60-day periods, distinct from the Part A sense of the term; don't conflate the two just because both are called "benefit period." |
| **grace period** | — | The window (often 3 months, for a Marketplace plan using the premium tax credit) an insurer must allow after a missed premium payment before ending coverage. Year-stable as a rule, unlike the dollar figures nearby it — `evals/gold/questions.yaml#hcg-10`. |
...
| **provider directory** | — | The plan's published list of its in-network providers, and the only authoritative answer to *"does this doctor take my plan?"*. Plan- and moment-specific, so it is a `structured_api` question and never a `reference` one — and since no provider-level data is ever vendored here (see **NPPES**), it is also the canonical thing the Phase 1a agent **abstains** on: `evals/gold/questions.yaml#abs-01`, which turns answerable at Phase 3. |
...
| **homebound** | — | The eligibility gate for home health care: either leaving home isn't medically recommended, or the patient has trouble leaving without help (a cane, wheelchair, walker, special transportation, or another person) because of illness or injury. Needing an aid to leave home satisfies this — it does not require being housebound entirely, a common misreading `evals/gold/questions.yaml#pub-04` is written to probe. |
...
| **late enrollment penalty** | — | A permanent premium surcharge for enrolling in Part B or Part D after first eligibility without other creditable coverage. The Part D version is computed from the **national base beneficiary premium**; see that entry for the exact formula. |
| **national base beneficiary premium** | — | The CMS-published figure ($38.99 for 2026) the Part D late enrollment penalty is computed from: 1% of this amount, times the number of full uncovered months, rounded to the nearest $0.10 and added to the premium for as long as Part D coverage continues. Changes every plan year — `evals/gold/questions.yaml#pub-03` marks it `plan_year: 2026` and `volatile: true` for exactly that reason, unlike the *shape* of the formula, which does not change. |
| **creditable (prescription drug) coverage** | — | Drug coverage expected to pay, on average, at least as much as standard Part D — the condition that avoids the late enrollment penalty. Having *any* drug coverage isn't sufficient; it must meet this bar (employer, union, TRICARE, VA, and Indian Health Service coverage typically qualify). |
...
| **LDCT** | Low Dose Computed Tomography | A chest CT scan run at reduced radiation settings, used for annual lung cancer screening in eligible current/former heavy smokers. `ncd_210.14`, one of the "additional preventive services" NCDs CMS can add under §1861(ddd) on a USPSTF grade A/B recommendation — see `evals/gold/questions.yaml#ncd-02`. |
| **cLBP** | chronic Lower Back Pain | The NCD's own defined term for the acupuncture-coverage indication in `ncd_30.3.3` — pain lasting 12+ weeks, with no identifiable systemic cause, not tied to surgery or pregnancy. Coverage under this NCD (up to 12 visits/90 days, up to 20/year) applies only to cLBP as defined here, not lower back pain generally. |
| **AHI / RDI** | Apnea-Hypopnea Index / Respiratory Disturbance Index | The per-hour rate of breathing interruptions a sleep study measures; `ncd_240.4` (CPAP for obstructive sleep apnea) conditions coverage on an AHI/RDI threshold from either a facility polysomnogram or a qualifying home sleep test. |
